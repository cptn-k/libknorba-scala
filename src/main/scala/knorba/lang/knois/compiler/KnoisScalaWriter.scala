...
import knorba.lang.common.syntax.{CustomTypeDescriptor, GridTypeDescriptor, PrimitiveType, PrimitiveTypeDescriptor, TypeDescriptor}
import knorba.lang.knois.compiler.KnoisScalaWriter.{Indent, KTYPE, NO_INDENT}
import knorba.lang.knois.syntax.{EnumerationTypeSpec, ParamDef, RecordTypeSpec, Service, Spec}
...
object KnoisScalaWriter {
  private class Indent(val n: Int) {
    private val str = 1.to(n)
      .foldLeft(new StringBuilder)((b, n) => b.append("    "))
      .toString()
...
class KnoisScalaWriter(os: OutputStream) {
  ...
  def writeEnumType(in: Indent, service: Option[UString], spec: EnumerationTypeSpec): Unit = {
    ...
    spec.members
      .indices
      .foreach(i => pw.printf(in + "val %s = new %s(%d.toByte)\n",
  ...
  def writeRecord(in: Indent, service: Option[UString], spec: RecordTypeSpec): Unit = {
    ...
    pw.println()
    writeRecordClass(in, service, spec)
  }
  ...
  def scalaNameFor(typeSpec: Option[TypeDescriptor]): UString = typeSpec match {
    ...
    case Some(s: GridTypeDescriptor) => U"KGrid"
  ...
  def writeRecordClass(in: Indent, service: Option[UString], spec: RecordTypeSpec): Unit = {
  ...
  def fieldToStr(in: Indent, param: ParamDef): String =
    in.toString + (param.typeDesc match {
      case Some(p: CustomTypeDescriptor) => param.identifier.name
  ...
  def writeGetter(in: Indent, i: Int, paramDef: ParamDef): Unit = {
    pw.printf(in + "def get%s: %s = get(%d).%s\n",
