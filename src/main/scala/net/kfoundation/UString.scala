...
object UString {
  ...
  private def wrap[T](a: Array[T]): Seq[T] =
  ...
  def of(octets: Seq[Byte], offset: Int, size: Int): UString = {
    ...
    val n = octets.drop(offset).copyToArray(selection)
    assert(n == selection.length)
...
class UString private (private val octets: Array[Byte]) {
  ...
  def append(raw: Seq[Byte]): UString = new UString(octets.concat(raw))
